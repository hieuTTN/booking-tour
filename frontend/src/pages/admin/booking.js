...
import { getMethod ,postMethod,deleteMethod,postMethodPayload} from '../../services/request';
...
const AdminBooking = ()=>{
    ...
    useEffect(()=>{
        const getBooking= async() =>{
            var response = await getMethod('http://localhost:8080/api/booking/admin/all-room');
        ...
        const getService= async() =>{
            var response = await getMethod('http://localhost:8080/api/services/public/findAll');
            var list = await response.json();
            setItemService(list)
        };
        getService();
    ...
    async function locBk(){
        ...
        var response = await getMethod('http://localhost:8080/api/booking/admin/all-room?from='+from+'&to='+to);
        var list = await response.json();
        setItems(list)
    }
    ...
    async function updateStatus() {
        ...
        var res = await getMethod('http://localhost:8080/api/booking/admin/update-status?id=' + itemBookingId + '&status=' + sstname)
        if (res.status < 300) {
    ...
    async function getBookingRoomAndService(idBooking){
        ...
        var response = await getMethod('http://localhost:8080/api/booking-room/public/find-by-booking?id='+idBooking);
        var list = await response.json();
        setItemBookingRoom(list)

        var response = await getMethod('http://localhost:8080/api/booking-service/public/find-by-booking?id='+idBooking);
        var list = await response.json();
    ...
    async function deleteBkService(id){
        ...
        var response = await deleteMethod('http://localhost:8080/api/booking-service/admin/delete?id='+id);
        if(response.status < 300){
            toast.success("Xóa thành công");
            var response = await getMethod('http://localhost:8080/api/booking-service/public/find-by-booking?id='+itemBookingId);
            var list = await response.json();
            setItemBookingService(list)
            $('#example').DataTable().destroy();
            var response = await getMethod('http://localhost:8080/api/booking/admin/all-room');
    ...
    async function saveBookingService(){
        ...
        console.log(list);
        var response = await postMethodPayload('http://localhost:8080/api/booking-service/admin/create?id='+itemBookingId, list);
        if (response.status < 300) {
    ...
    async function deleteBooking(id, sttName){
        ...
        var response = await deleteMethod('http://localhost:8080/api/booking/admin/delete?id=' + id)
        if (response.status < 300) {
            toast.success("Thành công");
            var response = await getMethod('http://localhost:8080/api/booking/admin/all-room');
            var list = await response.json();
